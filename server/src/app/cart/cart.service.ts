...
export class CartService {
  ...
  async updateCartItem(
    ...
    if (!item) {
      throw new HttpException(
        I18nContext.current().t("errors.cart.productDoesNotExistInCart"),
        HttpStatus.BAD_REQUEST
      );
    }
  ...
  async getUserCart(user: UserSessionDto): Promise<CartEntity> {
    ...
    return await this.cartRepository.getCartByIdWithItems(userFromDB.cart_id);
  }
  ...
  async createCartItem(
    ...
    if (product.quantity < cartItemDto.quantity) {
      throw new HttpException(
        `${I18nContext.current().t("errors.products.productNotEnough")} ${
          product.quantity
        }`,
        HttpStatus.BAD_REQUEST
      );
