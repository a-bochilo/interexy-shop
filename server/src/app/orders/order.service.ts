...
export class OrderService {
  ...
  //   async createOrder(cart: CartDto, userId: string) {
  ...
  //       );
  //     });
  //     const orderItems = prodEntities.map((item, i) => {
  ...
  //       (acccumulator: number, item) => acccumulator + item.price * item.quantity,
  //       0
  //     );
  ...
  async createOrder(cart: CartSessionDto, userId: string): Promise<OrderDto> {
    ...
    const orderItems = await Promise.all(prodEntities.map(async (product, i) => {
      return await this.createOrderItem(order, product, cart.items[i].quantity);
    }));

    order.items =  orderItems;
    order.total = orderItems.reduce(
              (acccumulator: number, item) => acccumulator + item.product_price * item.product_quantity,
              0
            );
